 ...
 * @brief Demonstration of the USB Interface Manager for dual HID interfaces
 * 
...
{
    printf("Initializing USB Interface Manager Demo\n");
    ...
    printf("USB Interface Manager Demo initialized\n");
}
...
void usb_interface_manager_demo_task(void)
{
    ...
    if (!interfaces_configured && hurricane_hw_device_host_connected()) {
        printf("Host connected, configuring interfaces\n");
...
static int add_hid_interface(uint8_t interface_num, const uint8_t* report_descriptor, uint16_t descriptor_length)
{
    ...
    hurricane_interface_descriptor_t interface_desc = {
        ...
        .interface_subclass = 0x00,      // No subclass
    ...
    // Update the HID report descriptor

/**
 * @file host_handler.c
 * @brief Implementation of host mode handler for dual HID example
 */
...
{
    printf("[Host Handler] Initializing USB host handler\n");
    ...
    printf("[Host Handler] Registered HID device handler\n");
}
...
{
    printf("[Host Handler] Deinitializing USB host handler\n");
...
void host_handler_task(void)
{
    ...
    if (active_device_idx < 0) {
        if (hurricane_hw_host_device_connected()) {
            printf("[Host Handler] Device detected but not yet enumerated\n");
...
bool host_handler_send_hid_report(uint8_t report_id, const uint8_t* data, uint16_t length)
{
    ...
    if (!devices[active_device_idx].is_hid || devices[active_device_idx].endpoint_out == 0) {
        printf("[Host Handler] Device is not HID or has no OUT endpoint\n");
        return false;
    }
    ...
    if (report_id != 0) {
        ...
        if (!report_buffer) {
            printf("[Host Handler] Failed to allocate report buffer\n");
            return false;
        }
    ...
    if (result != 0) {
        printf("[Host Handler] Failed to send HID report, error %d\n", result);
        return false;
    }
...
bool host_handler_set_keyboard_leds(uint8_t leds)
{
    ...
    if (!devices[active_device_idx].is_hid) {
        printf("[Host Handler] Device is not a HID device\n");
        return false;
    }
    ...
    if (result != 0) {
        printf("[Host Handler] Failed to set keyboard LEDs, error %d\n", result);
        return false;
    }
...
uint16_t host_handler_get_report_descriptor(uint8_t* descriptor, uint16_t max_length)
{
    ...
    uint16_t actual_length = max_length;
    int result = hurricane_hw_host_control_transfer(
        &setup,
        descriptor,
        &actual_length
    ...
    if (result != 0) {
        printf("[Host Handler] Failed to get HID report descriptor, error %d\n", result);
        return 0;
    }
...
static void host_device_attached_callback(void* device_handle)
{
    printf("[Host Handler] USB device attached\n");
    ...
    if (device_idx < 0) {
        printf("[Host Handler] No free slots to track device\n");
        return;
    }
    ...
    if (!enumerate_device(device_handle, &device_info)) {
        printf("[Host Handler] Failed to enumerate device\n");
        return;
    }
    ...
    if (devices[device_idx].is_hid) {
        if (!configure_hid_device(&devices[device_idx])) {
            printf("[Host Handler] Failed to configure HID device\n");
        }
    }
}
...
static void host_device_detached_callback(void* device_handle)
{
    printf("[Host Handler] USB device detached\n");
...
static void host_data_callback(uint8_t endpoint, void* buffer, uint16_t length)
{
    printf("[Host Handler] Received %d bytes on endpoint 0x%02X\n", length, endpoint);
...
static bool enumerate_device(void* device_handle, usb_device_info_t* device_info)
{
    ...
    uint16_t actual_length = sizeof(device_descriptor);
    int result = hurricane_hw_host_control_transfer(
        &setup,
        device_descriptor,
        &actual_length
    ...
    if (result != 0 || actual_length < 18) {
        printf("[Host Handler] Failed to get device descriptor, error %d\n", result);
        return false;
    }
    ...
    if (device_descriptor[14] > 0) { // Product string index
        ...
        uint16_t string_length = sizeof(string_descriptor);
    ...
    uint16_t config_length = sizeof(config_descriptor);
    ...
    if (result == 0 && config_length >= 9) {
        ...
        if (total_length > 9 && device_info->interface_count > 0) {
            ...
            if (full_config) {
                ...
                if (result == 0 && config_length >= total_length) {
...
static bool configure_hid_device(usb_device_info_t* device_info)
{
    ...
    printf("[Host Handler] Configuring HID device\n");
    ...
    if (result != 0) {
        printf("[Host Handler] Failed to set configuration, error %d\n", result);
        return false;
    }
    ...
    if (result != 0) {
        printf("[Host Handler] Failed to set protocol, error %d (not fatal)\n", result);
    ...
    result = hurricane_hw_host_control_transfer(&setup, NULL, 0);
    if (result != 0) {
        printf("[Host Handler] Failed to set idle rate, error %d (not fatal)\n", result);
        // Continue anyway, as some devices might not support this
    }
    
    printf("[Host Handler] HID device configured successfully\n");
    return true;
}
...
static void process_hid_report(const uint8_t* report_data, uint16_t length, uint8_t endpoint)
{
    ...
    printf("[Host Handler] Processing HID report (%d bytes)\n", length);
    ...
    if (active_device_idx >= 0 && 
        ...
        if (length >= 3) {
            ...
            printf("[Host Handler] Mouse report: buttons=%02X, dx=%d, dy=%d\n", 
                   buttons, dx, dy);
        }
    }
    // If this is from a keyboard, interpret the data
        ...
        if (length >= 3) {
            ...
            printf("[Host Handler] Keyboard report: modifier=%02X, key1=%02X\n", 
                   modifier, keycode1);
        }
    }
...
static void print_device_info(const usb_device_info_t* device_info)
{
    ...
    printf("\n[Host Handler] USB Device Information:\n");

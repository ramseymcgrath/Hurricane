 ...
 * @brief Implementation of device mode configuration for dual HID example
 */
...
{
    printf("[Device Config] Initializing device configuration\n");
    ...
    if (!config_desc) {
        printf("[Device Config] Failed to build configuration descriptor!\n");
        return;
    }
    ...
    printf("[Device Config] Device configuration initialized\n");
}
...
{
    printf("[Device Config] Deinitializing device configuration\n");
    ...
    printf("[Device Config] Device configuration deinitialized\n");
}
...
void device_config_task(void)
{
    ...
    if (current_connection != device_connected) {
        if (current_connection) {
            printf("[Device Config] Host connected to device\n");
            ...
            if (!interfaces_configured) {
                ...
                printf("[Device Config] HID interfaces configured\n");
            }
        } else {
            printf("[Device Config] Host disconnected from device\n");
        }
    ...
    if (device_connected && interfaces_configured) {
        ...
        if (hid_configs[0].configured && (current_time - last_mouse_time > 1000)) {
            // Move cursor in a circle pattern
            static int angle = 0;
        ...
        if (hid_configs[1].configured && (current_time - last_keyboard_time > 3000)) {
            ...
            printf("[Device Config] Sending keyboard report for '%c'\n", chars[char_index]);
...
{
    printf("[Device Config] Device configuration set to %d\n", configuration);
}
...
{
    printf("[Device Config] Interface %d alternate setting changed to %d\n", interface, alt_setting);
}
...
int device_config_send_mouse_report(int8_t dx, int8_t dy, uint8_t buttons)
{
    ...
    if (result != 0) {
        printf("[Device Config] Failed to send mouse report, error %d\n", result);
    }
...
int device_config_send_keyboard_report(uint8_t modifier, const uint8_t keycodes[6])
{
    ...
    if (result != 0) {
        printf("[Device Config] Failed to send keyboard report, error %d\n", result);
    }
...
static bool hid_control_callback(hurricane_usb_setup_packet_t* setup, void* buffer, uint16_t* length)
{
    ...
    if ((setup->bmRequestType & 0x60) == 0x20) { // Class request
        ...
        printf("[Device Config] HID control request 0x%02X for interface %d\n", setup->bRequest, interface_num);
        ...
        if (setup->bRequest == 0x01) { // GET_REPORT
            ...
            printf("[Device Config] GET_REPORT: type %d, id %d\n", report_type, report_id);
        ...
        if (setup->bRequest == 0x0B) { // SET_PROTOCOL
            ...
            printf("[Device Config] SET_PROTOCOL: interface %d to protocol %d\n", interface_num, protocol);
            return true;
        }
    }
...
static int add_hid_interface(uint8_t interface_num, const uint8_t* report_descriptor, uint16_t descriptor_length, uint8_t protocol)
{
    printf("[Device Config] Adding HID interface %d (protocol %d)\n", interface_num, protocol);
    ...
    if (result != 0) {
        printf("[Device Config] Failed to add interface %d, error %d\n", interface_num, result);
        return result;
    }
    ...
    if (result != 0) {
        printf("[Device Config] Failed to configure endpoint for interface %d, error %d\n", interface_num, result);
        return result;
    }
    ...
    printf("[Device Config] Successfully configured HID interface %d\n", interface_num);
    return 0;
}

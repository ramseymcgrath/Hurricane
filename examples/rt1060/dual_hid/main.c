/**
 * @file main.c
 * @brief Dual HID example application using Hurricane USB Interface Manager
 *
...
{
    printf("\n==================================================\n");
    printf("Hurricane Dual USB Stack - HID Example\n");
    printf("==================================================\n\n");
    ...
    printf("\n[Main] Setting up USB device mode...\n");
    device_config_init();
    
    // Initialize host mode handler
    printf("\n[Main] Setting up USB host mode...\n");
    host_handler_init();
    ...
    printf("* Device mode: Composite HID (Mouse + Keyboard)\n");
    printf("* Host mode: HID device detection and handling\n\n");
...
static void app_run_iteration(void)
{
    ...
    if (current_time - last_status_time > 5000) {
        ...
        if (host_handler_is_device_connected() && host_handler_get_device_info(&device_info)) {
            printf("  Host Mode: Connected to %s (VID: %04X, PID: %04X)\n", 
                device_info.product_name[0] ? device_info.product_name : "USB Device",
                device_info.vendor_id, device_info.product_id);
        } else {
            printf("  Host Mode: No Device Connected\n");
        }

/**
 * @file main.c
 * @brief Main entry point for the RT1060 dual USB stack example
 * 
 * This example demonstrates using both USB host and device stacks simultaneously
 * on the RT1060 platform with dynamic interface configuration.
 */
...
int main(void) {
    // Initialize hardware
    hurricane_hw_init();
    printf("🚀 Hurricane dual USB stack booted on RT1060\n");
    
    // Initialize the interface manager
    hurricane_interface_manager_init();
    printf("Interface manager initialized\n");
    
    // Configure device mode (USB2 controller)
    configure_device_mode();
    
    // Configure host mode (USB1 controller)
    configure_host_mode();
...
static void hid_attach_device(void* device) {
    ...
    // In a real implementation, we would set up the device and start reading HID reports
}
...
static void hid_data_callback(uint8_t endpoint, void* buffer, uint16_t length) {
    ...
    // In a real implementation, we would process the HID report here
    if (length >= 3) {
